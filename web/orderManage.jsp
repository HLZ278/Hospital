...
<%--            <c:forEach var="orderManageViews" items="${ orderManageViews }">--%>
<%--                <tr>--%>
<%--                    <td>${orderManageViews.hospitalID}</td>--%>
<%--                    <td>${orderManageViews.hospitalName}</td>--%>
<%--                    <td>${orderManageViews.grade}</td>--%>
<%--                    <td>${orderManageViews.area}</td>--%>
<%--                    <td>${orderManageViews.address}</td>--%>
<%--                    <td>${orderManageViews.releaseTime}</td>--%>
<%--                    <td>${orderManageViews.stopTime}</td>--%>
<%--&lt;%&ndash;                    <c:choose>&ndash;%&gt;--%>
<%--&lt;%&ndash;                        <c:when test="${hospitalIDMap.get(hospital.hospitalID)!=0}"><td>${hospitalIDMap.get(hospital.hospitalID)}</td></c:when>&ndash;%&gt;--%>
<%--&lt;%&ndash;                        <c:otherwise><td><a href="JavaScript:addHospitalUser(${hospital.hospitalID})">添加</a></td></c:otherwise>&ndash;%&gt;--%>
<%--&lt;%&ndash;                    </c:choose>&ndash;%&gt;--%>
<%--                    <td><a href="#" onclick='updateHospital(this)'>修改</a></td>--%>
<%--                </tr>--%>
<%--            </c:forEach>--%>
        </table>
